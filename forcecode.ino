...
Servo clawServo;
const int clawPin = 5; // Use PWM pin 5
...
void loop() {
    if (Serial.available() > 0) {
        ...
        if (cmd.equals("OPEN_CLAW")) {
            Serial.println("Opening claw");
            clawServo.write(90); // Open position
        } else if (cmd.equals("CLOSE_CLAW")) {
            Serial.println("Closing claw");
            clawServo.write(0); // Closed position
        } else if (cmd.equals("water")) {
            Serial.println("Starting water sequence");
            clawServo.write(90); // Open claw
            Serial.println("Claw opened");
            
            lineTrace(); // Line trace until stop
            
            Serial.println("Closing claw");
            clawServo.write(0); // Close claw
            
            Serial.println("Reversing for 1 second");
            robotReverse(); // Reverse for 1 second
            delay(1000);
            robotStop();
            
            Serial.println("Turning 180 degrees");
            robotTurnRight(); // Turn 180 degrees
            delay(1000); // Adjust delay as needed
            robotStop();
            
            Serial.println("Line tracing back");
            lineTrace(); // Line trace back
            
            Serial.println("Opening claw");
            clawServo.write(90); // Open claw
            
            Serial.println("Reversing away for 1 second");
            robotReverse(); // Reverse away
            delay(1000);
            robotStop();

            Serial.println("Water sequence complete");
        } else if (cmd.equals("milk")) {
            Serial.println("Starting milk sequence");
            clawServo.write(90); // Open claw
            Serial.println("Claw opened");
            ...
            Serial.println("Milk sequence complete");
        }
        if (cmd.equals("burger") || cmd.equals("sandwich") || cmd.equals("hotdog")){
            lineTracing();
